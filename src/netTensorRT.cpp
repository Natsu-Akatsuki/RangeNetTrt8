...
/**
 * @brief      Project a pointcloud into a spherical projection
 * image.projection.
 *
...
NetTensorRT::doProjection(const std::vector<float> &scan,
  ...
  float fov = std::abs(fov_down) +
              std::abs(fov_up); // get field of view total in radians
  ...
  for (size_t idx : orders) {
    ...
    std::vector<float> input = {ranges[idx], xs[idx], ys[idx], zs[idx],
                                intensitys[idx]};
  ...
  for (uint32_t i = 0; i < inputs.size(); ++i) {
    range_image[int(sorted_proj_ys[i] * _img_w + sorted_proj_xs[i])] =
        inputs[i];
  }
 ...
 * @param[in]  scan, LiDAR scans; num_points, the number of points in this scan.
 *
 * @return     Semantic estimates with probabilities over all classes
 * (_n_classes, _img_h, _img_w)
 */
std::vector<std::vector<float>>
NetTensorRT::infer(const std::vector<float> &scan, const uint32_t &num_points) {
    ...
    for (int i = 0; i < _img_d; i++) {
      ...
      if (!all_zeros) {
        projected_data[pixel_id][i] =
            (projected_data[pixel_id][i] - this->_img_means[i]) /
            this->_img_stds[i];
      }

      int buffer_idx = channel_offset * i + pixel_id;
      ((float *)_hostBuffers[_inBindIdx])[buffer_idx] =
          projected_data[pixel_id][i];
    }
  }
  ...
  // clock now
  if (_verbose) {
    std::cout << "Time for preprocessing: " << toc() * 1000 << "ms"
              << std::endl;
    tic();
  }
  ...
  if (_verbose) {
    ...
    std::cout << "Time for copy in: " << toc() * 1000 << "ms" << std::endl;
    tic();
  }
  ...
  if (_verbose) {
    CUDA_CHECK(cudaStreamSynchronize(_cudaStream));
    std::cout << "Time for inferring: " << toc() * 1000 << "ms" << std::endl;
    tic();
  }
  ...
  if (_verbose) {
    std::cout << "Time for copy back: " << toc() * 1000 << "ms" << std::endl;
    tic();
  }
  ...
  for (int pixel_id = 0; pixel_id < channel_offset; pixel_id++) {
    for (int i = 0; i < _n_classes; i++) {
      ...
      range_image[pixel_id].push_back(
          ((float *)_hostBuffers[_outBindIdx])[buffer_idx]);
    }
  }

  if (_verbose) {
    std::cout << "Time for taking the data out: " << toc() * 1000 << "ms"
              << std::endl;
    tic();
  }
